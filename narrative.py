...
def synthesize_narrative(timeline, stats):
    ...
    if total == 1:
        headline = "최초 작성 후 변경되지 않은 원형(Origin) 코드"
        first_ref = ""
        if first.get("ref_items"):
            ref = first["ref_items"][0]
            first_ref = f" (연관 {ref.get('type', '참조')} #{ref.get('number')}: '{ref.get('title')}')"
        body = (
            f"{first.get('date', '초기')} 커밋({first.get('hash')})에서 '{first.get('message', '')}' 목적으로 "
            f"처음 작성되었으며, 이후 한 번도 수정되지 않은 채 원형 그대로 유지되고 있습니다{first_ref}."
        )
    ...
    if reverts:
        ...
        body = (
            f"{first.get('date')} 커밋({first.get('hash')})에서 처음 도입된 후, "
            f"수정 과정에서 부작용 등으로 인해 롤백({revert_hashes})된 이력이 있습니다."
            f"{last_context} 최종 커밋({last.get('hash')})에서 문제가 보완되어 현재의 안전한 형태로 정착되었습니다."
        )
        return headline, body
    ...
    body = (
        f"{first.get('date')}에 처음 작성된 이후, 총 {total}단계의 변경 과정을 거쳤습니다. "
        f"{last.get('date')} 최근 커밋({last.get('hash')})에서 '{last.get('message')}' 작업이 수행되며{ref_part} "
        f"현재의 로직 구조로 보완되었습니다."
    )
    return headline, body